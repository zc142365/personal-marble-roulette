...
export class Roulette extends EventTarget {
  ...
  private _targetTime: number = 0; // simulation time we should have reached (ms)
  private _simTime: number = 0; // simulation time actually reached (ms)
  private _startAt: number | null = null; // start of the shared timeline; null means running solo
  private _pendingRemovals: { id: number; at: number }[] = [];
  ...
  private _update() {
    ...
    if (this._startAt !== null) {
      // Shared session: the wall clock decides how far to advance, so every client
      // has run the same number of steps at the same moment
      this._targetTime = syncNow() - this._startAt;
    } else {
      // Solo: catch up by at most 100ms of backlog (original behaviour)
      this._targetTime = Math.min(
        this._targetTime + delta * this._speed * this.fastForwarder.speed,
        this._simTime + 100
      );
    }

    // Per-frame step budget. A spectator joining late catches up here.
    let budget = 500;
    while (this._simTime + this._updateInterval <= this._targetTime && budget-- > 0) {
      this._simTime += this._updateInterval;
      this._flushRemovals();
      // Step size and sort point must depend on the step, never on the frame.
      // Doing either once per frame makes the outcome depend on device speed.
      this.physics.step((this._updateInterval / 1000) * this._timeScale);
  ...
  private _updateMarbles(deltaTime: number) {
    ...
    for (let i = 0; i < this._marbles.length; i++) {
      ...
      if (marble.y > this._stage.goalY) {
        ...
        // Removed on simulation time, not wall clock, so every client drops it
        // on exactly the same step
        this._pendingRemovals.push({ id: marble.id, at: this._simTime + 500 });
      }
    }
  ...
  private _flushRemovals() {
    ...
    this._pendingRemovals = this._pendingRemovals.filter((removal) => {
      if (removal.at > this._simTime) return true;
      this.physics.removeMarble(removal.id);
      return false;
    });
  }
  ...
  /**
   * @param startAt Shared start time (server clock). When given, physics advances
   *                from that moment on.
   */
  public start(startAt?: number) {
    ...
    if (startAt !== undefined) {
      ...
      this._simTime = 0;
      this._targetTime = 0;
      // Do not await the recorder: waiting would offset the start step per client
      if (this._autoRecording) this._recorder.start();
      this.physics.start();
  ...
  public setMarbles(names: string[]) {
    ...
    // Move the camera to the centre of the spawn area and zoom in
    if (totalCount > 0) {
